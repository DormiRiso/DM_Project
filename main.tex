...
Come prossimo passo abbiamo utilizzato diverse tecniche di classificazione con lo scopo di prevedere il possibile \textit{rating} di un gioco a partire dai suoi attributi. Abbiamo esplorato tecniche \textit{lazy} tipo \textit{kNN classification}, e tecniche \textit{eager} tipo \textit{naive Bayes} e \textit{decision trees}. L'analisi si è concentrata principalmente sulla classificazione della colonna \textit{rating} in base alle coppie di attributi \textit{AgeRec} con \textit{Weight} e \textit{NumDesires} con \textit{YearPublished}.

Per tutti gli algoritmi di classificazione abbiamo iniziato splittando il \textit{dataset} in due, mettendo da parte il $30\%$ da usare come \textit{dataset} di test mentre l'altro $70\%$ è stato usato per fare \textit{training} dei modelli. Il taglio è stato eseguito randomicamente per evitare effetti di \textit{bias} e per assicurarci che entrambi parti rappresentassero l'interità dei dati. 
\subsection{KNN}
...
Nel caso di due variabili indipendenti abbiamo utilizzato come variabile indipendente la colonna \textit{WeightedRating} con la speranza di poter predirre, tramite tecniche di regressione, l'eventuale successo di un gioco con determinate carateristiche. Abbiamo provato tutte le coppie tra le seguenti colonne: \textit{Weight}, \textit{Playtime}, \textit{AgeRec}, \textit{NumDesires}, \textit{LanguageEase}, trovando che i risultati migliori si trovavano per la coppia \textit{NumDesires} e \textit{Weight}. 
