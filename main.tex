...
Come ultima tecnica di classificazione abbiamo utilizzato i \textit{decision trees} per determinare il \textit{rating} di giochi. Come nelle sezioni precedenti, abbiamo diviso il \textit{dataset} in due parti (\textit{training} e \textit{test}), rimosso i \textit{NaN}, e standardizzato le colonne numeriche. Come nelle sezioni precedenti, si useranno le colonne \textit{NumDesires} e \textit{YearPublished} come \textit{feature} per classificare i giochi in base al \textit{target} di \textit{rating}.

\subsubsection{Selezione dei parametri}
Per ottimizzare le prestazioni e trovare i migliori valori per \textit{max depth}, \textit{min samples per leaf} \textit{min samples per split} e \textit{criterion} (tra \textit{gini} o \textit{entropy}, è stata effettuata una ricerca degli iperparametri tramite \textit{RandomizedSearchCV} di \textit{scikit-learn}, analizzando 500 combinazioni casuali attraverso una \textit{cross-validation} a 5 fold. Il processo ha permesso di individuare la configurazione ottimale (\textit{max depth}$=7$, \textit{min samples per leaf}$=100$, \textit{min samples per split}$=80$, \textit{criterion}$=$\textit{gini}), stabilizzando l'accuratezza sul test set al $60.6\%$. Questi risultati sono riportati in figura \ref{fig:dt_hyperP_tuning}.
...
\begin{figure}[h!]
	...
	\centering
	\caption{Risultati dell'ottimizzazione degli iperparametri tramite \textit{RandomizedSearchCV}. I grafici illustrano come la variazione dei parametri influenzi l'accuratezza media in \textit{Cross-Validation}. La classificazione è stata eseguita tramite un modello di \textit{decision trees} con lo scopo di classificare il \textit{rating} dei giochi tramite i features \textit{NumDesires} e \textit{YearPublished}.}
	\label{fig:dt_hyperP_tuning}
\end{figure}

Si nota anche che la scelta di \textit{criterion} tra \textit{entropy} e \textit{gini} è poco influente sullà bontà del modello. 


In aggiunta abbiamo studiato, al variare dei parametri, la complessità del modello al variare di certi parametri. Abbiamo fatto variare la profondità massima e \textit{min samples per leaf}, valutando come cambia l'accuratezza e il numero di nodi, ai fini di evitare \textit{overfitting} e controllare la complessità del modello. 
...
\begin{figure}[h!]
	...
	\centering
	\caption{Analisi riassuntiva della complessità del modello di \textit{decision trees} con lo scopo di classificare il \textit{rating} dei giochi tramite i features \textit{NumDesires} e \textit{YearPublished}.}
	\label{fig:dt_complexity_analysis}
\end{figure}

Il riassunto dell'analisi si trova in figura \ref{fig:dt_complexity_analysis}, dove si vede chiaramente che la scelta di \textit{max depth}$=7$ porta alla miglior accuratezza senza portare a problemi di \textit{overfitting} sul \textit{test set}, insieme a \textit{min samples per leaf}$=100$. Questa configurazione permette di mantenere il modello leggero e applicabile anche senza perdità di generalità su dati nuovi. 

\subsubsection{Analisi del modello}

Avendo ora fissato i migliori valori dei parametri, 
