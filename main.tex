...
È stato fatto anche un tentativo utilizzando il modello KNN invece che sulla colonna "Rating" su due elementi della colonna "Description", ovvero "Action" e "Roll"; per vedere se il modello fosse capace di classificare il genere di un videogioco. Per applicare il KNN sono state selezionate le colonne numeriche "Weight" e "AgeRec".

Per prima cosa è stato filtrato il dataset per mantenere solo quelle \textit{entries} che contengono almeno uno tra "Action" e "Roll" nella loro colonna "Description". Sono stati selezionati questi due descrittori perchè sono abbastanza popolati (5000 elementi nel \textit{training dataset} e 2000 nel \textit{test dataset}) e coprono principalmente due aree del piano distinte. 
...
Per il valore di k è stata seguita la stessa procedura di prima, arrivando ad utilizzare un valore di k pari a 30.
